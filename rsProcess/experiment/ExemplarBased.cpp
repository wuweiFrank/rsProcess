...
void ExemplarBased::ExemplarBased_GetPriority(float* imgData, int xsize, int ysize, vector<CPOINT> edge, CPOINT &pos)
{
	...
	for (int i = 0; i < edge.size(); ++i)
	{
		...
		double *np, *ip;
		np = ExemplarBased_Np(edge, edge[i], imgData, xsize, ysize);
		ip = ExemplarBased_Ip(imgData, edge[i], xsize, ysize);
...
void ExemplarBased::ExemplarBased_PriorityInpaint(CPOINT pos, int regionSize, float* imgData, float* mskData, int xsize, int ysize)
{
	...
	{
		for (int j = pos.y - regionSize; j <= pos.x + regionSize; ++j)
		{
	...
	double maxrel = -1; int maxidx = 0, maxidy=0;
	for (int i = 0; i < xsize - (2 * regionSize + 1); ++i)
	{
		for (int j = 0; j < ysize - (2 * regionSize + 1); ++j)
		{
			...
			if (!is)
			{
				float tmp=GetCoefficient(data1, data2, num);
				if (tmp>maxrel)
				{
					maxrel = tmp;
					maxidx = i; maxidy = j;
	...
	for (int i = -regionSize; i <=regionSize; ++i)
	{
		...
		{
			if (mskData[(j+pos.y)* xsize + i+pos.x] == -1)
			{
				imgData[(j + pos.y)* xsize + i + pos.x] = imgData[(j + maxidy)* xsize + i + maxidx];
			}
		}
	}
...
double* ExemplarBased::ExemplarBased_Np(vector<CPOINT> edge, CPOINT pos, float* imgData, int xisze, int ysize)
{
...
double* ExemplarBased::ExemplarBased_Ip(float* imgData, CPOINT pos, int xsize, int ysize)
{
	double direction[2];
	...
	direction[0] = Ix / sqrt(Ix*Ix + Iy*Iy);
	direction[1] = Iy / sqrt(Ix*Ix + Iy*Iy);
	return direction;
}